...
- **`deploy/changelog-he.json`** — שורה אחת בעברית לכל תג, מוצגת במסך "אודות" מתחת ל"בדוק עדכונים" במקום הודעת ה-commit הגולמית באנגלית. **חובה להוסיף שורה לכל תג חדש** (מפתח `vX.Y.Z`, ערך = תיאור קצר וברור למשתמש לא-טכני) — אחרת התג עדיין יופיע ברשימה, רק בלי תיאור.
