...
module.exports = function registerSmtp(app, db, deps) {
  ...
  app.put('/api/smtp/config', requireAdmin, (req, res) => {
    ...
    // All whitespace is stripped, not just trimmed: Google displays app passwords
    // as "abcd efgh ijkl mnop" and that's what gets pasted, but the 4 spaces are
    // presentational — SMTP AUTH needs the bare 16 characters.
    ...
    // Echo the mask back so the form can show "סיסמה נוכחית: ••••wxyz" right away
    // instead of still claiming "לא הוגדר עדיין" after a successful save.
    res.json({
      ok: true,
      password_masked: passwordToStore ? '••••' + passwordToStore.slice(-4) : '',
    });
  });
